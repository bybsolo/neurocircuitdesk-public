"""
demo_looming_circuit.py
-----------------------
Build, compile, and run a looming-detection circuit that extends the motion
pipeline (PR -> MVP -> ONOFF -> T4/T5) with an LPLC2-style looming detector
stage.  The looming detector receives directional motion signals from T4/T5
through four dendritic branches (a-d), each sampling a ring-3 neighbourhood
around a displaced cardinal direction, and multiplies them to produce a
...
Signal flow:
    input -> PR_col (divisive normalisation)
               ...
               MVP  (lateral neighbourhood feedback)
                |
    PR -> ONOFF_col (bandpass -> ON/OFF rectifiers)
              |           |
        MOTION_ON    MOTION_OFF  (Borst T4/T5)
              |           |
              +-----+-----+
                    |
                 LOOMING  (LPLC2: 4 dendrites x axon)

Usage:
    python demo_looming_circuit.py                 # default: 1261 columns
    python demo_looming_circuit.py --n-cols 547    # smaller test

Requires:
    data/highway.h5  (shipped with this demo)

Outputs (written to outputs/looming_demo/):
...
OUTDIR     = os.path.join(_THIS_DIR, 'outputs', 'looming_demo')
...
def make_looming_detector_template(canvas: Canvas):
    """LPLC2-style looming detector with four cardinal dendritic branches.

    Each branch (a-d) samples a ring-3 neighbourhood centered on a displaced
    cardinal point (offsets 32, 23, 19, 28 in the ring-3 spiral ordering
    around the MC center).  The four branch outputs are multiplied together
    by the axon block to produce a looming-selective signal.
    """
    def looming_detector_template(mc: MicroCircuit, neighborhood: Dict[int, int]):
        ordered_cols = canvas.graph_utils.local_order(
            mc.col_idx, num_rings=3, require_in_graph=False)

        # Four cardinal sub-neighbourhoods (ring-3 around displaced centers)
        branch_centers = {'a': 32, 'b': 23, 'c': 19, 'd': 28}
        branch_cols = {}
        for label, offset in branch_centers.items():
            if offset < len(ordered_cols):
                branch_cols[label] = canvas.graph_utils.local_order(
                    ordered_cols[offset], num_rings=3, require_in_graph=False)
            else:
                branch_cols[label] = ordered_cols  # fallback

        # Build input port names per branch
        branch_inputs = {}
        for label, cols in branch_cols.items():
            branch_inputs[label] = [f'input_{label}_col_{c}' for c in cols]

        # Add dendrite blocks (summation) and axon block (multiplication)
        for label in 'abcd':
        ...
        mc.specify_io(
            inputs=([(name, 'layer_a', name) for name in branch_inputs['a']] +
                    [(name, 'layer_b', name) for name in branch_inputs['b']] +
                    [(name, 'layer_c', name) for name in branch_inputs['c']] +
                    [(name, 'layer_d', name) for name in branch_inputs['d']]),
            outputs=[("output_looming", "looming_detector_block", "output")],
        )
    return looming_detector_template
...
@unified_algorithm
def lplc2_dendrite_algorithm_demo(inputs, params):
    """Dendrite branch: sum all directional motion inputs."""
    total = 0.0
...
@unified_algorithm
def lplc2_axon_algorithm_demo(inputs, params):
    """Axon: multiply the four branch outputs for looming selectivity."""
    a = inputs['input_a']
    b = inputs['input_b']
    c = inputs['input_c']
    d = inputs['input_d']
...
def build_and_compile(n_cols: int):
    ...
    for t in ('PR_col', 'MVP', 'ONOFF_col',
              'MOTION_ON_col', 'MOTION_OFF_col', 'LOOMING'):
        cv.add_mc_type(t)
    ...
    # 4. Motion detectors (ring-1). Inter-columnar at the data-flow level —
    # each cell reads centre + 6 ring-1 inputs. Centres + neighbourhoods
    # are shared between the ON and OFF stacks.
    ...
    # 5. Looming detectors (ring-6, sparser tiling)
    looming_template = make_looming_detector_template(cv)
    ...
    for col_idx, neighborhood in lplc2_centers.items():
        cv.add_microcircuit_intercolumnar(
            ...
            neighborhood=neighborhood, template=looming_template)

    # 6. Configure functions/params
    ...
    for mc in cv.mc_types['LOOMING']:
        mc.set_block_func('layer_a', lplc2_dendrite_algorithm_demo)
        mc.set_block_func('layer_b', lplc2_dendrite_algorithm_demo)
        mc.set_block_func('layer_c', lplc2_dendrite_algorithm_demo)
        mc.set_block_func('layer_d', lplc2_dendrite_algorithm_demo)
        mc.set_block_func('looming_detector_block', lplc2_axon_algorithm_demo)
    ...
    print("Wiring MOTION -> LOOMING ...")
    for looming_mc in cv.mc_types['LOOMING']:
        cols_by_layer = defaultdict(list)
        for name in looming_mc.input_ports.keys():
            parts = name.split('_', 3)
            layer_type = parts[1]
            col_id = int(parts[3])
            cols_by_layer[layer_type].append(col_id)
        for label in 'abcd':
            cv.connect_utils.mimo(
                'MOTION_ON_col', f'output_{label}', 'LOOMING', f'input_{label}_col',
                dst_center_col_idx=looming_mc.col_idx,
                cols=cols_by_layer[label], skip_viz=True)
            cv.connect_utils.mimo(
                'MOTION_OFF_col', f'output_{label}', 'LOOMING', f'input_{label}_col',
                dst_center_col_idx=looming_mc.col_idx,
                cols=cols_by_layer[label], skip_viz=True)
...
def main():
    ...
    parser.add_argument('--n-cols', type=int, default=1261,
                        help='Number of retinotopic columns (default: 1261)')
    args = parser.parse_args()
    n_cols = args.n_cols

    os.makedirs(OUTDIR, exist_ok=True)

    print(f"\n== Looming circuit demo  N_COLS={n_cols} ==\n")
    ...
    # LOOMING MCs use sparse tiling -- probe manually into a correctly-sized array
    looming_mcs = cv.mc_types['LOOMING']
    max_loom_col = max(mc.col_idx for mc in looming_mcs) + 1
    T_steps = lum.shape[0]
    lplc2 = np.zeros((T_steps, max_loom_col))
    for t_idx, yk in enumerate(prog.outputs):
        for mc in looming_mcs:
            key = (mc.name, "output_looming")
            if key in yk:
                lplc2[t_idx, mc.col_idx] = float(yk[key])

    # Save arrays
    tag = f'n{n_cols}'
    for name, arr in [('am_T', am_T), ('on', on), ('off', off),
                      ('T4a', T4a), ('T5a', T5a), ('lplc2', lplc2)]:
        np.save(os.path.join(OUTDIR, f'{name}_{tag}.npy'), arr)

    # Visualise
    coords = load_hex_positions(n_cols)
    frame = min(75, lum.shape[0] - 1)

    scatter_row(coords,
                panels=[("ON", on[frame]), ("OFF", off[frame])],
                suptitle=f"ON / OFF @ t={frame}  (N={n_cols})",
                path=os.path.join(OUTDIR, f'onoff_frame{frame}_{tag}.png'),
                vmin=0, vmax=0.25)

    scatter_row(coords,
                panels=[("T4a (ON motion)", T4a[frame]),
                        ("T5a (OFF motion)", T5a[frame])],
                suptitle=f"Motion detectors @ t={frame}  (N={n_cols})",
                path=os.path.join(OUTDIR, f'motion_frame{frame}_{tag}.png'),
                vmin=0, vmax=0.25)

    # LPLC2 uses sparse tiling -- extract only the populated columns
    lplc2_col_indices = sorted(mc.col_idx for mc in cv.mc_types['LOOMING'])
    lplc2_coords = load_hex_positions(max(lplc2_col_indices) + 1)
    lplc2_xy = lplc2_coords[lplc2_col_indices]
    lplc2_vals = lplc2[frame, lplc2_col_indices]
    vmax_lplc2 = max(1.0, float(np.nanmax(lplc2_vals))) if len(lplc2_vals) > 0 else 1.0
    scatter_row(lplc2_xy,
                panels=[("LPLC2 looming", lplc2_vals)],
                suptitle=f"LPLC2 looming detector @ t={frame}  (N={n_cols})",
                path=os.path.join(OUTDIR, f'lplc2_frame{frame}_{tag}.png'),
                vmin=0, vmax=vmax_lplc2)
